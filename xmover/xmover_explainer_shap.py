...
class XMoverWrapper():
    ...
    def load_mapping(self):
        ...
        for layer_num in [12]:
            print(layer_num)
            bam_path = os.path.join(mapping_dir,'layer-{}'.format(layer_num), 'europarl-v7.{}-{}.2k.{}.BAM.map'.format(self.src_lang, self.tgt_lang, layer_num))
            print(bam_path)
            gbdd_path = os.path.join(mapping_dir,'layer-{}'.format(layer_num), 'europarl-v7.{}-{}.2k.{}.GBDD.map'.format(self.src_lang, self.tgt_lang, layer_num))
            print(gbdd_path)
    ...
    def __call__(self, translations):
        ...
        print(len(translations))
        print("len xmovers")
        print(len(np.array(xmoverscores)))
